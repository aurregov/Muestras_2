HA$PBExportHeader$w_copia_mta_ref.srw
$PBExportComments$Para hacer la copia de muestras a referencias
forward
global type w_copia_mta_ref from w_sheet
end type
type cbx_red from checkbox within w_copia_mta_ref
end type
type dw_s_muestrario_color from u_dw_application within w_copia_mta_ref
end type
type dw_s_precio_vta from u_dw_application within w_copia_mta_ref
end type
type dw_dt_ref_vta from u_dw_application within w_copia_mta_ref
end type
type tab_1 from tab within w_copia_mta_ref
end type
...
type dw_ref_pdn_mue from u_dw_application within tabpage_6
end type
type tabpage_6 from userobject within tab_1
dw_ref_pdn_mue dw_ref_pdn_mue
end type
type tab_1 from tab within w_copia_mta_ref
tabpage_2 tabpage_2
tabpage_1 tabpage_1
...
type dw_head from u_dw_application within w_copia_mta_ref
end type
type cbx_ft from checkbox within w_copia_mta_ref
end type
type cbx_mo from checkbox within w_copia_mta_ref
end type
type cbx_mp from checkbox within w_copia_mta_ref
end type
type gb_2 from u_gb_base within w_copia_mta_ref
end type
type gb_1 from u_gb_base within w_copia_mta_ref
end type
end forward
...
global w_copia_mta_ref w_copia_mta_ref
...
public function integer of_validarsoloref ()
public function integer of_grabarsoloref ()
public function integer of_verificar_referencia_muestra (long a_fabrica, long a_linea, long a_referencia, long a_talla)
public function integer of_verificar_muestra_ref (long a_fabrica, long a_linea, long a_muestra, long a_talla)
public function integer of_grabarsolocol ()
end prototypes
...
For i=1 To ll_tot
	...
	ll_sw_lista   = tab_1.tabpage_3.dw_colores.GetItemNumber(i,"sw_lista")
...
//		*/						
		ls_ean = ''
//		IF ll_sw_lista = 1 THEN
//			ls_ean = tab_1.tabpage_3.dw_colores.GetItemString(i,"ean")
//			IF Trim(ls_ean) = '' OR Trim(ls_ean) = '0' THEN
//				ll_return =  luo_comun.of_generar_ean(ll_fabricaVta, ll_lineaVta,  n_cst_application.itr_appl)
//				IF ll_return = -1 THEN
//					RETURN -1
//				ELSEIF ll_return = 0 THEN
//					ls_mensaje = luo_comun.of_getstring(1)
//					MessageBox("Atenci$$HEX1$$f300$$ENDHEX$$n",ls_mensaje)
//					RETURN -1
//				ELSE
//					ls_ean = luo_comun.of_getstring(1)
//				END IF
//			END IF
//		END IF
		dw_dt_ref_vta.SetItem(ll_pos,"sw_lista", ll_sw_lista)
		
      tab_1.tabpage_3.dw_colores.SetItem(i,"ean", ls_ean )
		//dw_dt_ref_vta.SetItem(ll_pos,"ean", ls_ean )
		...
		/*
		* --------------------------------------------------------------------------
		* MODIFICADO: Agosto 08 de 2011 - ohlondon
		* --------------------------------------------------------------------------
      * se adicionan el campo co_familia  que debe ser replicado a dt_ref_vta.
		* --------------------------------------------------------------------------
		*/ 
		
		dw_dt_ref_vta.SetItem(ll_pos,"co_familia",&
		tab_1.tabpage_3.dw_colores.GetItemNumber(i,"co_familia"))
	End If
Next
...
/*
* ----------------------------------------------------------------------------
* MODIFICADO: Enero 25 de 2012 - ohlondon
* ----------------------------------------------------------------------------
* Se debe poner el sw_costo en uno al registro que tenga mayor porcentaje
* de distribucion...
* ----------------------------------------------------------------------------
*/
tab_1.tabpage_5.dw_dt_vta_pdn.SetSort('ca_dstrbcion D')
tab_1.tabpage_5.dw_dt_vta_pdn.Sort()
lde_dist=tab_1.tabpage_5.dw_dt_vta_pdn.SetItem(1,"sw_costo", 1)
...
long ll_fabrica,ll_linea,ll_referencia,ll_talla,ll_calidad,ll_res, ll_co_composicion
...
io_copiar.of_setComposicion(ll_co_composicion)
...
//2011-02-03
tab_1.tabpage_4.dw_recurso.InsertRow(0)
tab_1.tabpage_6.dw_ref_pdn_mue.SetTransObject(n_cst_application.itr_appl)
tab_1.tabpage_6.dw_ref_pdn_mue.InsertRow(0)
end subroutine
...
long ll_fabricaPdn,ll_lineaPdn,ll_refPdn,ll_tallaPdn

ll_fabricaPdn= tab_1.tabpage_6.dw_ref_pdn_mue.GetItemNumber(1,"co_fabrica_ref")
ll_lineaPdn  = tab_1.tabpage_6.dw_ref_pdn_mue.GetItemNumber(1,"co_linea_ref")
ll_refPdn    = tab_1.tabpage_6.dw_ref_pdn_mue.GetItemNumber(1,"co_referencia_ref")
ll_tallaPdn  = tab_1.tabpage_6.dw_ref_pdn_mue.GetItemNumber(1,"co_talla_ref")


If IsNUll(ll_fabricaPdn) Then
	MessageBox("Advertencia!","Debe digitar la fabrica de la referencia.",StopSign!)
	Return -1
End If

If IsNUll(ll_lineaPdn) Then
	MessageBox("Advertencia!","Debe digitar la linea de la referencia.",StopSign!)
	Return -1
End If

If IsNUll(ll_refPdn) Then
	MessageBox("Advertencia!","Debe digitar el c$$HEX1$$f300$$ENDHEX$$digode la referencia.",StopSign!)
	Return -1
End If

If IsNUll(ll_tallaPdn) Then
	MessageBox("Advertencia!","Debe digitar la talla de la referencia.",StopSign!)
	Return -1
End If
...
public function integer of_grabarsoloref ();DateTime ldt_fecha
String ls_usuario,ls_instancia
...
tab_1.tabpage_6.dw_ref_pdn_mue.SetItem(1,"co_fabrica",ll_fabrica)
tab_1.tabpage_6.dw_ref_pdn_mue.SetItem(1,"co_linea",ll_linea)
tab_1.tabpage_6.dw_ref_pdn_mue.SetItem(1,"co_muestra",ll_muestra)
tab_1.tabpage_6.dw_ref_pdn_mue.SetItem(1,"co_talla",ll_talla)

If tab_1.tabpage_6.dw_ref_pdn_mue.of_DatosActualizacion(ldt_fecha,ls_usuario,ls_instancia)<>1 Then
	Return -1
End If

If tab_1.tabpage_6.dw_ref_pdn_mue.Update(True,False)<>1 Then
	RollBack Using n_cst_application.itr_appl ;
	Return -1
End If

//tab_1.tabpage_6.dw_ref_pdn_mue.Reset()
tab_1.tabpage_6.dw_ref_pdn_mue.InsertRow(0)
...
WHERE co_referencia_ref= :a_referencia AND  
      co_talla_ref = :a_talla 
  using n_cst_application.itr_appl    ;
...
Long 		ll_tot, ll_return
long 		ll_fabricaVta, ll_lineaVta
long 		ll_fila, ll_sw_lista
String 	ls_ean, ls_mensaje
...
ll_tot        = tab_1.tabpage_3.dw_colores.RowCount()
ll_fabricaVta = dw_head.getitemNumber(1, "co_fabrica")
ll_lineaVta   = dw_head.getitemNumber(1, "co_linea")

For ll_fila=1 To ll_tot
	ll_sw_lista   = tab_1.tabpage_3.dw_colores.GetItemNumber(ll_fila,"sw_lista")

	/*
	* --------------------------------------------------------------------------
	* MODIFICADO: Enero 24 de 2011 - ohlondon
	* --------------------------------------------------------------------------
	* se adicionan la generacion de eanes.
	* --------------------------------------------------------------------------
	*/
	ls_ean = ''
//	IF ll_sw_lista = 1 THEN
//		ls_ean = tab_1.tabpage_3.dw_colores.GetItemString(ll_fila,"ean")
//		IF Trim(ls_ean) = '' OR Trim(ls_ean) = '0' THEN
//			ll_return =  luo_comun.of_generar_ean(ll_fabricaVta, ll_lineaVta,  n_cst_application.itr_appl)
//			IF ll_return = -1 THEN
//				RETURN -1
...
//		END IF
//	END IF
	dw_dt_ref_vta.SetItem(ll_fila,"sw_lista", ll_sw_lista)
	dw_dt_ref_vta.SetItem(ll_fila,"ean", ls_ean )
Next

Return 1
end function

on w_copia_mta_ref.create
int iCurrent
call super::create
this.cbx_red=create cbx_red
...
this.Control[iCurrent+1]=this.cbx_red
this.Control[iCurrent+2]=this.dw_s_muestrario_color
this.Control[iCurrent+3]=this.dw_s_precio_vta
this.Control[iCurrent+4]=this.dw_dt_ref_vta
this.Control[iCurrent+5]=this.tab_1
this.Control[iCurrent+6]=this.dw_head
this.Control[iCurrent+7]=this.cbx_ft
this.Control[iCurrent+8]=this.cbx_mo
this.Control[iCurrent+9]=this.cbx_mp
this.Control[iCurrent+10]=this.gb_2
this.Control[iCurrent+11]=this.gb_1
end on

on w_copia_mta_ref.destroy
call super::destroy
if IsValid(MenuID) then destroy(MenuID)
destroy(this.cbx_red)
...
n_tr_application	ltr_produrif
ltr_produrif = CREATE n_tr_application	
...
If ltr_produrif.of_connectdb( ls_dbms, ls_database, ls_user, ls_passwd, &
										 ls_servername, ls_lock) <> 0 Then										 
	CLOSE (THIS)
	RETURN  
END IF
 
DataWindowChild ldwc_familia, ldwc_familia1

tab_1.tabpage_3.dw_colores.GetChild('co_familia',ldwc_familia)
ldwc_familia.SetTransObject(ltr_produrif)
ldwc_familia.Retrieve()
...
long    	ll_res,ll_tot, ll_recurso_minimo, ll_ref_vta , ll_fila, ll_filas
long    	ll_cliente,ll_sucursal,ll_cant ,ll_recurso_mues_bas,ll_recurso_mat_prima
String  	ls_tipoMuestra
Integer	li_fabrica_vta, li_linea_vta, li_talla_vta    
...
Long ll_co_fabrica, ll_co_linea, ll_co_muestra, ll_co_talla
int  li_co_recurso

ll_co_fabrica = lo_param.il_vector[1]
ll_co_linea = lo_param.il_vector[2]
ll_co_muestra = lo_param.il_vector[3]
ll_co_talla = lo_param.il_vector[4]

SELECT co_recurso  
  INTO :li_co_recurso
  FROM dt_muestras
 WHERE ( dt_muestras.co_fabrica = :ll_co_fabrica) AND  
       ( dt_muestras.co_linea = :ll_co_linea  ) AND  
       ( dt_muestras.co_muestra = :ll_co_muestra ) AND  
       ( dt_muestras.co_talla = :ll_co_talla  ) 
	USING n_cst_application.itr_appl;	
...
If IsNull(li_co_recurso)   Then
	MessageBox("Advertencia!","La muestra no tiene recurso .",StopSign!)
	Return 
End If
...
//////////////////////////////////////////////////////////////////////////
 ...
 /// GAC , si solo tiene la relacion dt_muestras_ref,  se debe poder trar para modificar la composicion si se requiere
/*
* ----------------------------------------------------------------------------
* MODIFICADO: Mayo 16 de 2011 - gagudelo - ohlondon
* Segun SOL# 45105 solo debe validar la fabrica 5
* ----------------------------------------------------------------------------
If ( ll_co_fabrica = 5 or & 
     ( ( ll_co_fabrica = 2 or ll_co_fabrica = 8 ) and &
	    ( li_co_recurso = 2100 or li_co_recurso = 2080 ) )	) THEN 
		 //debe verificar  todo 
*/
If ( ll_co_fabrica = 5 ) THEN 
...
ll_tot=tab_1.tabpage_6.dw_ref_pdn_mue.Retrieve(lo_param.il_vector[1],& 
				  	                lo_param.il_vector[2],&
					                lo_param.il_vector[3],&
					                lo_param.il_vector[4])
...
Else
	...
	If ll_tot = 0 then
   	tab_1.tabpage_6.dw_ref_pdn_mue.InsertRow(0)
	End If
	tab_1.SelectTab("tabpage_6") 
	tab_1.tabpage_6.dw_ref_pdn_mue.SetFocus()
end if
end event
...
Long li_tipoprod
String ls_ruta_ini, ls_diseno

///GAC  octubre 21 de 2010,  
///Si la fabrica de la muestra = 5 O fabrica  es 2 o 8 y el recurso de la muestra es 2010 y 2080
/// grabar todo 
///Sino pregunte, si solo graba la relacion dt_Muestras_ref  



Long ll_co_fabrica, ll_co_linea, ll_co_muestra, ll_co_talla,ll_cont,ll_recurso_minimo, ll_co_talla_ref
integer li_co_recurso
string ls_dificultad
...
If ( ll_co_fabrica = 5 ) THEN 
		 //debe grabar todo
Else
	If of_validarsoloref()<>1 Then
	 Return
   End If
	If of_grabarsoloref()<>1 Then
...
//Pasar a la Red
IF cbx_red.Checked = TRUE THEN
	ls_diseno = dw_head.GetItemString(1,"co_diseno_muestra")
	
	ls_diseno = Trim(ls_diseno)
	
	s_base_parametros lo_param

   lo_param.cadena[1] = ls_diseno
	
	OpenWithParm(w_pasar_diseno_red, lo_param)
...
IF (NOT IsNull(ll_fabrica) and ll_fabrica > 0 ) AND &
   (NOT IsNull(ll_linea) and ll_linea > 0 ) AND &
	...
	ll_res = of_verificar_referencia_muestra(ll_fabrica, ll_linea, ll_referencia, ll_talla)
	IF ll_res = -1 THEN
		RETURN 2
	ELSEIF ll_res = 1 THEN
		MessageBox("Advertencia!","La referencia ya existe para otra muestra.",Exclamation!)
		Return 2
	END IF
...
on tabpage_6.create
this.dw_ref_pdn_mue=create dw_ref_pdn_mue
this.Control[]={this.dw_ref_pdn_mue}
end on

on tabpage_6.destroy
destroy(this.dw_ref_pdn_mue)
end on
...
integer height = 1036
integer taborder = 70
string dataobject = "d_m_ref_pdn_mue"
boolean vscrollbar = false
boolean livescroll = false
...
AcceptText()
ll_fabrica=GetItemNumber(1,"co_fabrica_ref")
ll_linea=GetItemNumber(1,"co_linea_ref")
ll_referencia=GetItemNumber(1,"co_referencia_ref")
ll_talla=GetItemNumber(1,"co_talla_ref")
ll_calidad = 1

//messagebox("esta ref",ll_referencia)
...
If dwo.name="co_fabrica_ref" Then
	ll_fabrica=Long(Data)
ElseIf dwo.name="co_linea_ref" Then
	ll_linea=Long(Data)
ElseIf dwo.name="co_referencia_ref" Then
	ll_referencia=Long(Data)
ElseIf dwo.name="co_talla_ref" Then
	ll_talla=Long(Data)
end If

Long ll_co_fabrica_mue, ll_co_linea_mue, ll_co_muestra, ll_co_talla_mue 

ll_co_fabrica_Mue = dw_head.getitemNumber(1,"co_fabrica")
ll_co_linea_mue = dw_head.getitemNumber(1,"co_linea")
ll_co_muestra = dw_head.getitemNumber(1,"co_muestra")
ll_co_talla_mue = dw_head.getitemNumber(1,"co_talla") 
/*
If dwo.name="cs_talla" Then
	ll_dato=Long(Data)
	ll_res=lo_comun.of_nombreMTallas(ll_fabrica,ll_linea,ll_talla,ll_dato,&
...
If dwo.name="co_talla_ref" Then
	ll_dato=Long(Data)
	ll_res=lo_comun.of_nombreMuestraTalla(ll_fabrica,ll_linea,ll_co_muestra,ll_dato,&
	                                 n_cst_application.itr_appl)
												
	If ll_res<>1 Then
		MessageBox("Advertencia!","C$$HEX1$$f300$$ENDHEX$$digo de talla no v$$HEX1$$e100$$ENDHEX$$lido.",StopSign!)
		Return 2
	End If
	SetItem(1,"de_talla",lo_comun.of_getstring(1))
End If
...
If dwo.name="cs_talla" Then
	lo_param.il_vector[1]=GetItemNumber(1,"co_fabrica_ref")
	lo_param.il_vector[2]=GetItemNumber(1,"co_linea_ref")
	lo_param.il_vector[3]=GetItemNumber(1,"co_talla_ref")
...
If dwo.name="co_composicion" Then
	...
	lo_param=Message.PowerObjectParm  ///envia parametros ente venta y ventana
...
event itemfocuschanged;call super::itemfocuschanged;//SelectText(1,4)
end event
...
////	tab_1.selectTab(tab_1.tabpage_6)
////	Return 1
////end if
//
//Send(Handle(this),256,9,Long(0,0))
