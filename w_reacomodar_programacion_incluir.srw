HA$PBExportHeader$w_reacomodar_programacion_incluir.srw
$PBExportComments$Obj.Ventana en la que se consulta la lista de $$HEX1$$f300$$ENDHEX$$rdenes que se est$$HEX1$$e100$$ENDHEX$$n procesando y las cuales son susceptibles de ser movidas por la reacomodaci$$HEX1$$f300$$ENDHEX$$n de prioridades.Se despliega la lista y se selecciona opci$$HEX1$$f300$$ENDHEX$$n donde se procesan + muestran con nuevo orden pr$$HEX3$$4441542a00c2$$ENDHEX$$
forward
global type w_reacomodar_programacion_incluir from w_sheet
end type
type pb_refresh from picturebutton within w_reacomodar_programacion_incluir
end type
type cbx_all from checkbox within w_reacomodar_programacion_incluir
end type
type dw_tipoprod from datawindow within w_reacomodar_programacion_incluir
end type
type cb_cancelar from u_cb_application within w_reacomodar_programacion_incluir
end type
type mle_1 from multilineedit within w_reacomodar_programacion_incluir
end type
type cb_2 from u_cb_application within w_reacomodar_programacion_incluir
end type
type cb_1 from u_cb_application within w_reacomodar_programacion_incluir
end type
type cb_reacomodar from u_cb_application within w_reacomodar_programacion_incluir
end type
type dw_ordenes from u_dw_application within w_reacomodar_programacion_incluir
end type
type gb_orden from u_gb_base within w_reacomodar_programacion_incluir
end type
end forward
...
global w_reacomodar_programacion_incluir w_reacomodar_programacion_incluir
...
PRIVATE:
	...
	Long il_nu_orden,il_fabrica,il_linea,il_muestra, il_tipoprod
...
on w_reacomodar_programacion_incluir.create
int iCurrent
call super::create
...
on w_reacomodar_programacion_incluir.destroy
call super::destroy
