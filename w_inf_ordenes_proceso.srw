HA$PBExportHeader$w_inf_ordenes_proceso.srw
$PBExportComments$Window Class para el reporte de las $$HEX1$$f300$$ENDHEX$$rdenes de producci$$HEX1$$f300$$ENDHEX$$n que se encuentran en estado de proceso
forward
global type w_inf_ordenes_proceso from w_report
end type
type gb_ordenes from u_gb_base within w_inf_ordenes_proceso
end type
end forward
...
integer width = 3557
integer height = 1960
string title = "Informe de $$HEX1$$f300$$ENDHEX$$rdenes en proceso"
string menuname = "m_inf_ordenes_proceso"
boolean resizable = false
gb_ordenes gb_ordenes
end type
global w_inf_ordenes_proceso w_inf_ordenes_proceso
...
* PURPOSE	 :	Recupera los datos de las $$HEX1$$f300$$ENDHEX$$rdenes relacionadas con el filtro de busqueda,
					para el reporte de las $$HEX1$$f300$$ENDHEX$$rdenes en proceso.
* PRECOND.	 : None
* POSTCOND.	 : None
* DEVELOPER	 : Carlos Andr$$HEX1$$e900$$ENDHEX$$s Rico
* DATE		 :	14 de Enero de 2005
...
********************************************************************************************************/
//VARIABLE DECLARATION
Long ll_i,ll_col,ll_rtncode,ll_dias, ll_horas
DatawindowChild ldwc_data
DateTime ldtm_feiniact, ldtm_fefinact
//BEGIN SCRIPT-----------------------------------------------------------------------------------------------------
ll_rtncode = dw_report.GetChild('dw_inf_ordenes_proceso',ldwc_data)
...
If ll_col > 0 Then
		For ll_i=1 To ll_col
				...
				If Not IsNull(ldtm_feiniact) AND Not IsNull(ldtm_feiniact) Then
						ll_dias=DaysAfter(Date(ldtm_feiniact),Date(ldtm_fefinact))+1
						...
						If ll_dias > 1 Then
								...
								End If
						End If
						ldwc_data.SetItem(ll_i,'dias_teoricos',ll_dias)
				End If			
		Next
End If
////------------------------------------------------------------------------------------------------------
...
on w_inf_ordenes_proceso.create
int iCurrent
call super::create
...
on w_inf_ordenes_proceso.destroy
call super::destroy
...
type dw_report from w_report`dw_report within w_inf_ordenes_proceso
integer x = 69
integer y = 92
integer width = 3378
integer height = 1608
string dataobject = "d_inf_ordenes_proceso_cmp"
boolean border = true
...
integer width = 3451
integer height = 1716
string text = "Informe de las $$HEX1$$f300$$ENDHEX$$rdenes en proceso"
end type
