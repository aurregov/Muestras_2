HA$PBExportHeader$n_cst_programacion_ordenes_old.sru
$PBExportComments$Esta es la clase encargada de la programaci$$HEX1$$f300$$ENDHEX$$n de ordenes
forward
global type n_cst_programacion_ordenes_old from nonvisualobject
end type
end forward

global type n_cst_programacion_ordenes_old from nonvisualobject autoinstantiate
end type
...
PROTECTED:
		...
		Long		il_orden,il_estado,il_grupo,il_maquina,il_recurso,il_fabrica//Grupo, M$$HEX1$$e100$$ENDHEX$$quina que se va a asignar a la tarea
...
public function integer of_actualizartarea (long al_nu_orden, long al_consec, long al_grupo, long al_cola_grupo, long al_fabrica, long al_maqtin, long al_cola_mq_tin)
...
public function integer of_programarorden (long al_orden, datetime adtm_fecha_inicio);/*-------------------------------------------------
	...
	Date		: Lunes 06 de Diciembre de 2004.H:11:40
---------------------------------------------------
La fecha y hora para programar, debe ser una fecha futura,
si es menor que el momento en que se hace la programaci$$HEX1$$f300$$ENDHEX$$n,
se debe tomar el momento de programaci$$HEX1$$f300$$ENDHEX$$n.
No se debe tomar un fecha u hora pasadas.
//Se recupera la lista de tareas de la orden
...
//			End Choose							
			...
			li_res=of_actualizartarea(il_orden,ll_consec,il_grupo,ii_cola_grupo,ll_null/*il_fabrica*/,&
										ll_null/*il_maquina*/,ll_null/*ii_cola_maquina*/)
			If  li_res<>1 Then
				Return li_res
			End If
...
//////////////////////////////////////////////////////////////////////////
			...
			If ll_can>0 Then
				If UpperBound(il_vector_grupos_examina)<=0 Then
					...
					String(il_recurso)+", puede ser que este no sea de la planta de desarrollo.",StopSign!)
					Return -1
				End If
			End If
				
			//////////////////////////////////////////////////////////////////////////
			// JFVD:Si no se tiene ningun dato en la interseccion, entonces se colocan todos
			// los de la tarea
			//////////////////////////////////////////////////////////////////////////
			If UpperBound(il_vector_grupos_examina)<=0 Then
...
End Choose
//-------------------------------------------------
ll_retrieve=ids_estado_tarea_grupo.Retrieve()
Choose Case ll_retrieve
	Case Is < 0
...
ids_grupos_busca.Dataobject='d_grupos_seleccion'
...
For ll_i=1 To ll_lim_grupos		
	...
	For ll_c=1 To ll_cantcolas_grupo //Recorrido por el numero de colas del grupo determinado				
		...
		If ids_grupos_busca.RowCount()>0 Then //Si existen registros para este grupo y cola en las tareas
			ids_grupos_busca.SetSort('f_mov_ini A')
			ids_grupos_busca.Sort()
			/*Se verifica si la tarea que esta programando se puede colocar antes
			de la primera de la lista. Se estima el tiempo entre el par$$HEX1$$e100$$ENDHEX$$metro fecha_hora
			inicio y la fecha de inicio del primer registro de la cola filtrada */										
			...
			//-------------------------------------------------
			ldtm_fe_cola=ids_grupos_busca.GetItemDateTime(1,'f_mov_ini')
			//Si la fechahora obtenida es inferior a la de inicio del primer registro
			If ldtm_fe_fin_estimada < ldtm_fe_cola Then
			...
			Else//Si no hay tiempo antes de la primera tarea y se encuentra entre las registradas
				lb_out=False
				ll_k=ids_grupos_busca.RowCount()-1
				For ll_j=1 to ll_k
					ldtm_fe_finmedio=ids_grupos_busca.GetItemDateTime(ll_j,'f_mov_fin')

					ldtm_fe_ini_estimada=ldtm_fe_finmedio
					ll_res=This.of_addminturno(ldtm_fe_ini_estimada/*ref datetime adtm_fechahora*/,&
													  ll_turno_grupo/*long al_turno*/,&
													  ade_tiempo_tarea/*decimal ade_minutos */)
					...
					ldtm_fe_cola=ids_grupos_busca.GetItemDateTime(ll_j+1,'f_mov_ini')
					If ldtm_fe_fin_estimada < ldtm_fe_cola Then
						ll_pos_ext=ids_lista_grupos_colas.InsertRow(0)
						ids_lista_grupos_colas.SetItem(ll_pos_ext,'id_grupo',ll_grupo)
						ids_lista_grupos_colas.SetItem(ll_pos_ext,'cola',ll_c)									  									  
				...
				If Not lb_out Then
					// Si la tarea se debe adicionar con al final de la cola
					/*Se adiciona un registro en Ds_lista_grupos_colas con el grupo y la cola que se est$$HEX1$$e100$$ENDHEX$$n procesando,
					con la fecha y hora de finalizaci$$HEX1$$f300$$ENDHEX$$n de la $$HEX1$$fa00$$ENDHEX$$ltima tarea.*/
					ll_k=ids_grupos_busca.RowCount()
					ldtm_fe_cola=ids_grupos_busca.GetItemDateTime(ll_k,'f_mov_fin')
					...
					ids_lista_grupos_colas.SetItem(ll_pos_ext,'fecha_hora_ini',ldtm_fe_ini_estimada)
					ids_lista_grupos_colas.SetItem(ll_pos_ext,'fecha_hora_fin',ldtm_fe_fin_estimada)
				End If//FIN.Si la tarea se debe adicionar con al final de la cola
			End If//FIN.Si hay tiempo antes de la primera tarea
				
		Else //ELSE. Si existen registros para este grupo y cola en las tareas
			ldtm_fe_ini_estimada=adtm_fecha_hora_inicio
			ll_res=This.of_addminturno(ldtm_fe_ini_estimada/* ref datetime adtm_fechahora*/,&
												ll_turno_grupo/*long al_turno*/,&
												ade_tiempo_tarea/*decimal ade_minutos */)
			...
			ids_lista_grupos_colas.SetItem(ll_pos_ext,'fecha_hora_ini',ldtm_fe_ini_estimada)						
			ids_lista_grupos_colas.SetItem(ll_pos_ext,'fecha_hora_fin',ldtm_fe_fin_estimada)
			//**********Si no existe el grupo y la cola se asigna alli!									
		End If//FIN. Si existen registros para este grupo y cola en las tareas
...
ids_maquinas.of_load('d_ds_maquinas_colas_obtmaq')
...
For ll_i=1 To ll_rows
	...
	ls_where_maq +='(co_fabrica='+String(ll_fabrica)+' and cs_maquina='+String(ll_maquina)+') or '
Next
...
If Right(ls_where_maq,1)=")" Then		
	ls_modified_select="DataWindow.Table.Select='"+ls_select_ini+ls_where_maq+"'"
...
For ll_i=1 To ll_lim_maquinas		
	...
	For ll_c=1 To ll_cantcolas_maquina //Recorrido por el numero de colas de la m$$HEX1$$e100$$ENDHEX$$quina determinada				
		...
		ids_maquinas.Filter()
		If ids_maquinas.RowCount()>0 Then //Si existen registros para este grupo y cola en las tareas
			ids_maquinas.SetSort('f_mov_ini A')
			ids_maquinas.Sort()
			/*Se verifica si la tarea que esta programando se puede colocar antes
			de la primera de la lista. Se estima el tiempo entre el par$$HEX1$$e100$$ENDHEX$$metro fecha_hora
			inicio y la fecha de inicio del primer registro de la cola filtrada */						
			...
			ldtm_fe_cola=ids_maquinas.GetItemDateTime(1,'f_mov_ini')
			If ldtm_fe_fin_estimada < ldtm_fe_cola Then
			...
			Else//Si no hay tiempo antes de la primera tarea y se encuentra entre las registradas
				lb_out=False
				ll_k = ids_maquinas.RowCount()-1
				For ll_j=1 To ll_k
					//Iteraci$$HEX1$$f300$$ENDHEX$$n desde la primera m$$HEX1$$e100$$ENDHEX$$quina hasta la penultima
					ldtm_fe_finmedio=ids_maquinas.GetItemDateTime(ll_j,'f_mov_fin')
					...
					ldtm_fe_fin_estimada=idt_fechaHoraFinal
					//----------------------------------------------------------------
					ldtm_fe_cola=ids_maquinas.GetItemDateTime(ll_j+1,'f_mov_ini')
					If ldtm_fe_fin_estimada < ldtm_fe_cola Then											
						ll_pos_ext=ids_lista_maquinas_colas.InsertRow(0)
						ids_lista_maquinas_colas.SetItem(ll_pos_ext,'co_fabrica',ll_fabrica)
						ids_lista_maquinas_colas.SetItem(ll_pos_ext,'cs_maquina',ll_maquina)
						...
						EXIT
					End If
				Next//Iteraci$$HEX1$$f300$$ENDHEX$$n desde la primera m$$HEX1$$e100$$ENDHEX$$quina hasta la penultima
				
				If Not lb_out Then
					// Si la tarea se debe adicionar con al final de la cola
					/*Se adiciona un registro en Ds_lista_grupos_colas con el grupo y la cola que se est$$HEX1$$e100$$ENDHEX$$n procesando,
					con la fecha y hora de finalizaci$$HEX1$$f300$$ENDHEX$$n de la $$HEX1$$fa00$$ENDHEX$$ltima tarea.*/
					ll_k=ids_maquinas.RowCount()
					ldtm_fe_cola=ids_maquinas.GetItemDateTime(ll_k,'f_mov_fin')
					//---------------------------------------------------------
					ldtm_fe_ini_estimada=ldtm_fe_cola
		...
		Else //ELSE. Si no existen registros para esta m$$HEX1$$e100$$ENDHEX$$quina y cola en las tareas
			//-------------------------------------------------------------------
			ldtm_fe_ini_estimada=adtm_fecha_hora_inicio
			...
			ids_lista_maquinas_colas.SetItem(ll_pos_ext,'cs_maquina',ll_maquina)
		  	...
		  	ids_lista_maquinas_colas.SetItem(ll_pos_ext,'fecha_hora_ini',ldtm_fe_ini_estimada)																							
			ids_lista_maquinas_colas.SetItem(ll_pos_ext,'fecha_hora_fin',ldtm_fe_fin_estimada)						
		End If//FIN. Si existen registros para esta m$$HEX1$$e100$$ENDHEX$$quina y cola en las tareas
...
de los grupos
---------------------------------------------------*/
Long ll_len,ll_alp[],ll_i,ll_pos
...
ll_i=	iuo_comun.of_actualizar_tarea_color_ss(al_nu_orden, &
		...
		2/*Condicion:PROGRAMADA*/,n_cst_application.itr_appl)
////-------------------------------------------------
If ll_i<>1 Then 
...
long ll_tot,ll_filtrados,i,ll_tipo,ll_codigo
...
on n_cst_programacion_ordenes_old.create
call super::create
TriggerEvent( this, "constructor" )
end on

on n_cst_programacion_ordenes_old.destroy
