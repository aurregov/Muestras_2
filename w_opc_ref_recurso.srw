HA$PBExportHeader$w_opc_ref_recurso.srw
$PBExportComments$Permite seleccionar adicionalmente un recurso
forward
global type w_opc_ref_recurso from w_response
end type
type cb_buscar from commandbutton within w_opc_ref_recurso
end type
type dw_consulta from u_dw_application within w_opc_ref_recurso
end type
type cb_1 from commandbutton within w_opc_ref_recurso
end type
type cb_aceptar from commandbutton within w_opc_ref_recurso
end type
type dw_1 from u_dw_application within w_opc_ref_recurso
end type
type gb_1 from groupbox within w_opc_ref_recurso
end type
type gb_2 from u_gb_base within w_opc_ref_recurso
end type
end forward
...
global w_opc_ref_recurso w_opc_ref_recurso
...
If Right(a_where,1)=")" Then
	...
	ll_pos=pos(ls_sel1,"m_disenos.co_diseno )")
...
is_where="m_disenos.co_diseno ) and ("
String ls_cad
...
If Not IsNull(ls_cad) Then
	is_where+="dt_medidas_est_ref.co_diseno="+ls_cad+" and "
End If  
...
If Not IsNull(ls_cad) Then
	is_where+="dt_medidas_est_ref.co_recurso="+ls_cad+" and "
End If
...
integer height = 952
integer taborder = 30
string dataobject = "d_h_preref1_recurso"
