HA$PBExportHeader$w_inf_med_mue.srw
$PBExportComments$Para las medidas de las referencias con el nuevo esquema
forward
global type w_inf_med_mue from w_report
end type
type dw_head from u_dw_application within w_inf_med_mue
end type
type dw_tiempo from u_dw_application within w_inf_med_mue
end type
type dw_referencia from u_dw_application within w_inf_med_mue
end type
type gb_1 from groupbox within w_inf_med_mue
end type
type gb_2 from groupbox within w_inf_med_mue
end type
type gb_referencia from u_gb_base within w_inf_med_mue
end type
end forward
...
global w_inf_med_mue w_inf_med_mue
...
String is_where,is_where_copia, is_tipo_muestra

long il_fabrica,il_linea,il_referencia,il_talla,il_calidad, il_muestra
long il_tipoprod,il_recurso,il_diseno, il_grupo_tlla, il_fabrica_ref, il_linea_ref
...
If IsValid(io_param) Then
	...
	il_diseno       = io_param.il_vector[5]
	il_recurso      = io_param.il_vector[6]
	il_referencia   = io_param.il_vector[7] 
	il_grupo_tlla   = io_param.il_vector[8]
 	il_tipoprod     = io_param.il_vector[9] 
	il_fabrica_ref  = io_param.il_vector[10] 
	il_linea_ref    = io_param.il_vector[11]
	is_tipo_muestra = io_param.is_vector[1]

	dw_referencia.SetItem(1,"co_fabrica",il_fabrica)
	dw_referencia.setItem(1,"co_linea",il_linea)
	dw_referencia.setItem(1,"co_muestra",il_muestra)
	dw_referencia.setItem(1,"talla",il_talla)
	dw_referencia.setItem(1,"co_recurso",il_recurso)
	dw_referencia.setItem(1,"co_diseno",il_diseno)
	
	If il_fabrica = 5 Then
		dw_report.DataObject = 'd_inf_med_mue_tolerancia'
	Else
		dw_report.DataObject = 'd_inf_med_mue'
	End if
...
dw_report.height = newheight - 650
dw_report.SetRedraw(True)

end event

type dw_report from w_report`dw_report within w_inf_med_mue
integer x = 18
integer y = 600
integer width = 3625
integer height = 1208
integer taborder = 0
string dataobject = "d_inf_med_mue"
boolean border = true
...
long 		ll_res
String 	ls_talla, 	ls_nom_recurso
String 	ls_ref_ant
long 		ll_cliente,ll_sucursal
String 	ls_cliente
Decimal{2} lde_docenas
String ls_ref, ls_nombrediseno 
...
ls_talla= dw_referencia.GetItemString(1, 'de_talla')
Modify("talla.text='"+ls_talla+"'")

ls_nom_recurso = "("+String(il_recurso )+") "+dw_referencia.GetItemString(1, 'de_recurso_tejido')
Modify("recurso.text='"+ls_nom_recurso+"'")

ls_ref=String(il_fabrica_ref )+" "+&
       String(il_linea_ref )+" "+&
		 String(il_referencia)

Modify("referencia.text='"+ls_ref+"'")

ls_bpl_co_tipprod = GetItemString(1,"bpl_co_tipoprod")
...
ll_res=lo_comun.of_nombrecliente( ll_cliente, ll_sucursal, is_tipo_muestra,n_cst_application.itr_appl)
If ll_res=1 Then
	ls_cliente = lo_comun.of_getString(1)
	ls_cliente = "("+String(ll_cliente)+"-"+String(ll_sucursal)+") "+ls_cliente
Else
	ls_cliente = "("+String(ll_cliente)+"-"+String(ll_sucursal)+") "
End If
...
Modify("diseno.text='("+String(ll_diseno)+") "+dw_referencia.GetItemString(1, 'de_diseno' )+"'")
...
lds_1 = create n_ds_application
lds_1.DataObject="d_inf_det_mue_bas"
lds_1.SetTransObject(n_cst_application.itr_appl)

lds_1.Retrieve(il_recurso,&
					il_fabrica,&
					il_linea,&
					il_muestra,&
					il_talla,&
...
lds_2.DataObject="dgr_obs_dt_med_mue"
lds_2.SetTransObject(n_cst_application.itr_appl)

lds_2.Retrieve(il_recurso,&
					il_fabrica,&
					il_linea,&
					il_muestra,&
					il_talla,&
...
IF lds_2.RowCount() > 0 THEN
	ls_cad1  = lds_2.GetItemString(1,"de_observacion")
	IF Trim(String(ls_cad1,'')) <> '' THEN
		ls_obser += ls_cad1  
	ELSE
...
ll_res = lo_comun.of_nombreDisenoMuestra &
         ( il_fabrica, il_linea, il_muestra, il_talla, 1, n_cst_application.itr_appl)
	
If ll_res<>1 then
	Return 
End If	

ls_nombrediseno = lo_comun.of_getString(1)

ls_obser = "Dise$$HEX1$$f100$$ENDHEX$$o de la Muestra: "+ ls_nombrediseno+"~r~n~r~n"+ls_obser	
...
string dataobject = "d_m_producc_teo_mues"
boolean vscrollbar = false
end type
