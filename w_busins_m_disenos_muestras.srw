HA$PBExportHeader$w_busins_m_disenos_muestras.srw
$PBExportComments$Permite buscar e insertar partes de materia prima
forward
global type w_busins_m_disenos_muestras from w_base_busqueda_mod
end type
end forward
...
global w_busins_m_disenos_muestras w_busins_m_disenos_muestras
...
For i=1 to ll_tot
	...
	IF NOT IsNull(ls_diseno_muestra ) AND Trim(ls_diseno_muestra) <> '' THEN 
		...
		ElseIf ll_res = 1 Then
			SetNUll(ll_res)
			MessageBox("Advertencia!","El Dise$$HEX1$$f100$$ENDHEX$$o " + ls_diseno_muestra + " ya est$$HEX2$$e1002000$$ENDHEX$$asignado en otra muestra. Por favor verifique.",StopSign!)
			dw_1.SetItem(i,'co_diseno',ll_res)
			dw_1.SetFocus()
			dw_1.SetColumn('co_diseno')
			Return -1
		End If
	END IF
...
on w_busins_m_disenos_muestras.create
call super::create
end on

on w_busins_m_disenos_muestras.destroy
call super::destroy
end on
...
type st_1 from w_base_busqueda_mod`st_1 within w_busins_m_disenos_muestras
string text = "Digite el Dise$$HEX1$$f100$$ENDHEX$$o:"
end type

type sle_1 from w_base_busqueda_mod`sle_1 within w_busins_m_disenos_muestras
end type
...
type cb_adicionar from w_base_busqueda_mod`cb_adicionar within w_busins_m_disenos_muestras
integer x = 82
integer y = 1604
integer height = 100
integer taborder = 30
...
type cb_eliminar from w_base_busqueda_mod`cb_eliminar within w_busins_m_disenos_muestras
integer x = 430
integer y = 1604
integer height = 100
integer taborder = 40
...
type cb_aceptar from w_base_busqueda_mod`cb_aceptar within w_busins_m_disenos_muestras
integer x = 864
integer y = 1604
integer taborder = 50
end type
...
type cb_cancelar from w_base_busqueda_mod`cb_cancelar within w_busins_m_disenos_muestras
integer x = 1216
integer y = 1604
integer taborder = 60
end type

type dw_1 from w_base_busqueda_mod`dw_1 within w_busins_m_disenos_muestras
integer x = 50
integer width = 1541
integer height = 1240
integer taborder = 20
...
Long ll_cons, ll_cantDiseno, ll_diseno_utilizado,  ll_fila, ll_find
...
If (IsValid(lo_param) ) THEN 
	...
	ll_cantDiseno = UpperBound(lo_param.is_vector)
	ll_diseno_utilizado = This.Rowcount( )
	FOR ll_fila =1 TO ll_diseno_utilizado
		IF Trim(String(lo_param.is_vector[1],'')) = Trim(String(This.Object.de_diseno[ll_fila], '')) THEN
			MessageBox('Informaci$$HEX1$$f300$$ENDHEX$$n','El dise$$HEX1$$f100$$ENDHEX$$o '+String(lo_param.is_vector[1])+' ya fue utilizado. no sera ingresado.')
			This.deleterow(ll_diseno_utilizado)
			ll_diseno_utilizado = This.Rowcount( )
			ll_fila = ll_fila - 1				
			lb_salir = true
			EXIT
	...
	IF UpperBound(lo_param.is_vector) > 0 AND NOT lb_salir THEN
...
type gb_1 from w_base_busqueda_mod`gb_1 within w_busins_m_disenos_muestras
integer width = 1586
integer taborder = 0
string text = "Dise$$HEX1$$f100$$ENDHEX$$os de muestra"
end type

type gb_2 from w_base_busqueda_mod`gb_2 within w_busins_m_disenos_muestras
integer width = 1586
integer taborder = 0
