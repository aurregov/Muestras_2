HA$PBExportHeader$u_pb_calendar.sru
...
If Not isnull(ad_date) then 
	is_return_date = string(ad_date, 'yyyy/mm/dd')
End If
end subroutine
...
//if left(message.stringparm,8) = "calendar" then  // = "calendar MM/DD/YYYY"
	...
	CHOOSE CASE i_obj
		CASE Structure!
			...
			if not isvalid(i_dw) then 
				...
				closewithreturn(i_window,'1900/01/01')
			end if
			if i_dw.rowcount() >= il_row and il_row > 0 then
				...
				//messagebox('type',ls)
				if ls = 'date' then
					dateparm = string(i_dw.getitemdate(il_row,is_column_name),'YYYY/MM/DD')
				else
					dateparm = string(i_dw.getitemdatetime(il_row,is_column_name),'YYYY/MM/DD')
				end if
			else 
...
if ib_requester_is_datawindow = false then
	CHOOSE CASE i_obj
		CASE Structure!
			i_dw.SetText(String(id_date_selected,"YYYY/MM/DD"))
...
closewithreturn(i_window,string(id_date_selected, 'YYYY/MM/DD'))
end event
