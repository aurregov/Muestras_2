HA$PBExportHeader$w_med_ref_old.srw
$PBExportComments$Nuevas medidas de referencia
forward
global type w_med_ref_old from w_sheet
end type
type dw_panel from u_dw_application within w_med_ref_old
end type
type dw_med_rep from u_dw_application within w_med_ref_old
end type
type st_1 from statictext within w_med_ref_old
end type
type dw_detalle from u_dw_application within w_med_ref_old
end type
type dw_referencia from u_dw_application within w_med_ref_old
end type
type gb_referencia from u_gb_base within w_med_ref_old
end type
type gb_1 from u_gb_base within w_med_ref_old
end type
type gb_2 from u_gb_base within w_med_ref_old
end type
end forward
...
global w_med_ref_old w_med_ref_old
...
If dw_med_rep.rowCount()=0 Then
	lds_1.DataObject=dw_med_rep.DataObject
	lds_1.SetTransObject(n_cst_application.itr_appl)

   Messagebox('Var','ll_tot='+String(ll_tot)+'~n')
	ll_tot=lds_1.Retrieve(ll_fabrica,ll_linea,ll_referencia,ll_talla,ll_calidad,ll_diseno,ll_recurso)

	dw_med_rep.setRedraw(False)
	lds_1.RowsCopy(1,ll_tot,Primary!,dw_med_rep,1,Primary!)

	For i=1 to ll_tot
		dw_med_rep.setItem(i,"co_fabrica",il_fabrica)
		dw_med_rep.setItem(i,"co_linea",il_linea)
		dw_med_rep.setItem(i,"co_referencia",il_referencia)
		dw_med_rep.setItem(i,"co_talla",il_talla)
		dw_med_rep.setItem(i,"co_fabrica",il_fabrica)
		dw_med_rep.setItem(i,"co_calidad",il_calidad)
		dw_med_rep.setItem(i,"co_recurso",il_recurso)
		dw_med_rep.setItem(i,"co_diseno",il_diseno)
	Next

	of_verPulgadas()
	dw_med_rep.setRedraw(True)
End If
...
For i=1 To ll_tot
	...
	ll_f=dw_med_rep.InsertRow(0)
	dw_med_rep.SetItem(ll_f,"co_fabrica",il_fabrica)
	dw_med_rep.SetItem(ll_f,"co_linea",il_linea)
	dw_med_rep.SetItem(ll_f,"co_referencia",il_referencia)
	dw_med_rep.SetItem(ll_f,"co_tipoprod",il_tipoprod)
	dw_med_rep.SetItem(ll_f,"co_calidad",il_calidad)
	dw_med_rep.SetItem(ll_f,"id_med_rep",ls_med_rep)
	dw_med_rep.SetItem(ll_f,"co_talla",il_talla)
	dw_med_rep.SetItem(ll_f,"co_recurso",il_recurso)
	dw_med_rep.SetItem(ll_f,"co_diseno",il_diseno)
...
If dw_med_rep.accepttext()<>1 Then
...
ll_tot=dw_med_rep.RowCount()


For i=1 to ll_tot
	dw_med_rep.SetItem(i,"ca_pulg",lo_pulg.of_mmToPulg(&
	dw_med_rep.GetItemDecimal(i,"ca_cm")))	
Next
...
dw_med_rep.Retrieve(il_fabrica,il_linea,il_referencia,il_talla,il_calidad,il_diseno,il_recurso)
...
long ll_cant
  ...
  SELECT count(*)  
   ...
   WHERE ( dt_med_ref.co_centro = 12 ) AND  
         ...
         ( dt_med_ref.co_tipoprod = :il_tipoprod ) 
...
ll_tot=dw_med_rep.deletedcount()

For i=1 To ll_tot
	ls_med_rep=dw_med_rep.GetItemString(i,"id_med_rep",delete!,True)
...
public function integer of_grabar ();//////////////////////////////////////////////////////////////////////////
// JUAN FERNANDO VASQUEZ DUQE 
// Viernes 30 de Enero de 2004 HORA 14:48:29:306
// 
// 
//////////////////////////////////////////////////////////////////////////
...
If dw_med_rep.of_DatosActualizacion(ldt_fecha,ls_usuario,ls_instancia)<>1 Then
	Return -1
End If
...
this.Control[iCurrent+1]=this.dw_panel
this.Control[iCurrent+2]=this.dw_med_rep
this.Control[iCurrent+3]=this.st_1
this.Control[iCurrent+4]=this.dw_detalle
this.Control[iCurrent+5]=this.dw_referencia
this.Control[iCurrent+6]=this.gb_referencia
this.Control[iCurrent+7]=this.gb_1
this.Control[iCurrent+8]=this.gb_2
end on

on w_med_ref_old.destroy
call super::destroy
